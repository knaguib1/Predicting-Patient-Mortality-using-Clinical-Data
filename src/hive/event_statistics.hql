...
FROM (SELECT count(DISTINCT TIME) AS encounter_count
      FROM alive_events
      GROUP BY patient_id) encounter_metrics;
...
-- ***** your code below *****
FROM (SELECT datediff(to_date(max(time)), to_date(min(time))) as record_length
      FROM alive_events
      GROUP BY patient_id) length_metrics;
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE concat('%', 'DIAG', '%')
GROUP BY event_id
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE concat('%', 'LAB', '%')
GROUP BY event_id
...
FROM alive_events
-- ***** your code below *****
WHERE event_id LIKE concat('%', 'DRUG', '%')
GROUP BY event_id
